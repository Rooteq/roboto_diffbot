...
#include "roboto_diffbot/diffbot_system.hpp"
  ...
  const hardware_interface::HardwareInfo & info)
{
  ...
  wheel_r.setup(cfg.right_wheel_name, cfg.enc_counts_per_rev);
...
{
  comms.read_encoder_values(wheel_l.enc, wheel_r.enc);

  double delta_seconds = period.seconds();

  double pos_prev = wheel_l.pos;
  wheel_l.pos = wheel_l.calc_enc_angle();
  wheel_l.vel = (wheel_l.pos - pos_prev) / delta_seconds;

  pos_prev = wheel_r.pos;
  wheel_r.pos = wheel_r.calc_enc_angle();
  wheel_r.vel = (wheel_r.pos - pos_prev) / delta_seconds;

  return hardware_interface::return_type::OK;
}

hardware_interface::return_type roboto_diffbot ::DiffBotSystemHardware::write(
  const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
  comms.set_motor_values(wheel_l.cmd / wheel_l.rads_per_count / cfg.loop_rate, wheel_r.cmd / wheel_r.rads_per_count / cfg.loop_rate);
  return hardware_interface::return_type::OK;
}
...
PLUGINLIB_EXPORT_CLASS(
  roboto_diffbot::DiffBotSystemHardware, hardware_interface::SystemInterface)